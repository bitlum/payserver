...
func TestWalletSvrWsNtfns(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "btcdconnected",
			newNtfn: func() (interface{}, error) {
				return NewCmd("btcdconnected", true)
			},
			staticNtfn: func() interface{} {
				return NewBtcdConnectedNtfn(true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"btcdconnected","params":[true],"id":null}`,
			unmarshalled: &BtcdConnectedNtfn{
				Connected: true,
			},
		},
		{
		...
		},
		{
			name: "newtx",
			newNtfn: func() (interface{}, error) {
				return NewCmd("newtx", "acct", `{"account":"acct","address":"1Address","category":"send","amount":1.5,"bip125-replaceable":"unknown","fee":0.0001,"confirmations":1,"trusted":true,"txid":"456","walletconflicts":[],"time":12345678,"timereceived":12345876,"vout":789,"otheraccount":"otheracct"}`)
			},
			staticNtfn: func() interface{} {
